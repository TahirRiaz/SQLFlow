...
namespace SQLFlowCore.Services.Schema
{
    internal class SchemaSQL
    {
        ...
        internal SchemaSQL(SqlConnection trgSqlCon, SqlConnection srcSqlCon, string srcFile, string trgConString,
        ...
        )
        {
            ...
            _invalidChecksumDataTypes.Add("geometry");
            ...
            _ingoreChecksumColumns.Add("DataSet_DW");
        ...
        internal void Compare()
        {
            ...
            */
            ...
            foreach (var item in newColumns)
            {
                ...
                if (item.IsIgnored == 0 && _ignoreColumns.Contains(item.COLUMN_NAME, StringComparer.InvariantCultureIgnoreCase) == false)
                {
            ...
            foreach (var item in comonColumns)
            {
                ...
                if (item.COLUMN_NAME.Equals("HashKey_DW", StringComparison.InvariantCultureIgnoreCase))
                {
                ...
                switch (item.IsKeyColumn.ToString())
                {
                    case "0":
                        {
                            if (_hashKeyCols.Contains(item.COLUMN_NAME, StringComparer.InvariantCultureIgnoreCase) == false)
                            {
                                ValidUpdateColumns += $",[{item.COLUMN_NAME}]";
                            }

                            if (!_hashKeyCols.Contains(item.COLUMN_NAME, StringComparer.InvariantCultureIgnoreCase)
                                    && !_invalidChecksumDataTypes.Contains(item.DATA_TYPE_SRC, StringComparer.InvariantCultureIgnoreCase)
                                        && !_ingoreChecksumColumns.Contains(item.COLUMN_NAME, StringComparer.InvariantCultureIgnoreCase))
                ...
                if (item.IsDateColumn.ToString() == "1")
                {
                    DateColumnVerified = $"[{item.COLUMN_NAME}]";
                }
            ...
            else
            {
                ...
                foreach (var val in srcKeyColumns)
                    foreach (var item in comonColumns)
                        if (val.Equals(item.COLUMN_NAME, StringComparison.InvariantCultureIgnoreCase))
            ...
            else
            {
                ...
                foreach (var item in comonColumns)
                    if (_dateColumn.Equals(item.COLUMN_NAME, StringComparison.InvariantCultureIgnoreCase))
