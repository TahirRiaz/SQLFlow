...
Status: phase 1 shipped on `main` on 2026-09-11 (the queue moved into the control plane, the node protocol, the ownership lease, the tests); phases 2 and 3 remain. Written the same day against the code on `main` before the change, so "today" in sections 2 and 3 means the state this design replaced. Two decisions changed during implementation: the break-glass `sqlflow runs cancel --db` path stays (the dispatcher's reconcile makes a direct catalog cancel safe, so removing it would have cost an operator a tool for nothing), and the node registry is kept in memory and flushed to the catalog on a cadence rather than written per heartbeat, for the fleet-size target in section 4.11.
...
Phase 2 completes the protocol so the node needs nothing else: the execution spec includes what `RunClaimedAsync` joins today (repo name and remote, pipeline relative path, commit pin, flow version hash, run parameters, credential reference and username) plus the downstream watermark table and landing-reset verdict, resolved by the control plane at hand-out time with the same code that resolves them on the node today (it moves, it is not duplicated); `GET /api/v1/node/flow-versions/{hash}` serves the snapshotted YAML (content-addressed, cached on the node exactly as now); and `POST /api/v1/node/runs/{runId}/trace` accepts batched statements and events, which the existing sinks already buffer through a channel, so batching every 250 ms or 200 items is a change to their drain loop only.
...
The replica target already has one authoritative definition, `WorkerPoolStore.ResolveTarget`. The control plane exposes it at `GET /api/v1/nodes/pools/{pool}/scale-target` (node scope), computed from the queue's live counts and the leases' busy counts, and both deployments switch to the KEDA `metrics-api` scaler against that endpoint with a bearer `TriggerAuthentication` (Kubernetes) or a custom scale rule with a secret reference (Azure Container Apps). The `maxConcurrentRunsPerReplica` bicep parameter that "MUST match" the node's constant disappears: each node reports its slot count in its poll, so the target divides by what the fleet actually offers. The Go-driver connection secret and the duplicated SQL query are deleted.
...
| `src/SqlFlow.Node/RunWorker.cs` | Drain loop over `INodeTransport`; heartbeat loop and cancel polling replaced by the poll contract; phase 2 removes every `CatalogDbContext` use. |
| `src/SqlFlow.Node/CatalogRun*Sink.cs` | Phase 2: post batches to the trace endpoint instead of writing rows. |
...
| `deploy/bicep/worker.bicep`, `main.bicep`, `deploy/k8s/worker-pool.yaml`, `deploy/compose/docker-compose.yml`, `deploy/docker/worker-entrypoint.sh` | Worker env: `SQLFLOW_CONTROL_PLANE_URL` and `SQLFLOW_NODE_TOKEN` replace `SQLFLOW_CATALOG_DB`; scale rule swapped; scaler secret removed. |
...
2. **The node needs only the control plane.** Execution spec, flow-version fetch, trace batches, watermark and landing-reset resolution in the spec; `SqlFlow.Node` drops its `SqlFlow.Catalog` reference; `--db` leaves the worker verb.
3. **The platform follows.** `scale-target` endpoint and the KEDA `metrics-api` rules in bicep and Kubernetes; scaler secret removed; the GUI dispatch panel; reference pages, the wiki decision page, and the manifest rebuilt.
