...
/*
  ...
  -- Summary			:	The function returns a table variable (@Return) that contains information about the lineage of the specified starting object. 
							...
							It then uses a common table expression (CTE) to recursively build the lineage of the starting object by joining the flw.LineageEdge table. 
  ...
  ##################################################################################################################################################
*/
CREATE FUNCTION [flw].[LineageAfter]
(
    @StartObject NVARCHAR(255),
    @Expanded INT -- Shows a detailed view of the tree without clapsing each object to max level
)
...
*/
    ...
    BEGIN
        ...
        (
            SELECT src.*,
                   ROW_NUMBER() OVER (PARTITION BY src.FlowID,
                                                   src.ToObjectMK,
                                                   src.FromObjectMK
            ...
            FROM @Holder src
                ...
                (
                    ...
                    ON a.FlowID = src.FlowID
                       AND a.ToObjectMK = src.ToObjectMK
    ...
    BEGIN
        ...
        SELECT RecID,
               ...
               Circular,
               PathNum,
               ISNULL(Trg.Step, src.Step) Step,
               1 [Sequence],
               ISNULL(Trg.[Level], src.[Level]) [Level],
               MaxObjectLevel,
               Priority,
               NoOfChildren,
               MaxLevel,
...
END;
GO
EXEC sp_addextendedproperty N'MS_Description', '-- Purpose			:    "LineageAfter" that takes in two parameters: @StartObject, which is a string value representing the starting object for the lineage search, 
							 and @Expanded, which is an integer value indicating whether to return a simple or expanded result set.
  -- Summary			:	The function returns a table variable (@Return) that contains information about the lineage of the specified starting object. 
							
							The function first determines the starting object by checking the @StartObject parameter against various columns in the flw.LineageParsed table. 
							It then uses a common table expression (CTE) to recursively build the lineage of the starting object by joining the flw.LineageEdge table. 
							The resulting data is inserted into a table variable (@Holder), which is then used to populate the @Return table variable.
							...
							If @Expanded is 1, the table variable is returned with an expanded result set. If @Expanded is 2, 
							the table variable is returned with an expanded result set that includes all levels of the lineage. 
							The specific formatting differences for each option are described in the code.', 'SCHEMA', N'flw', 'FUNCTION', N'LineageAfter', NULL, NULL
GO
