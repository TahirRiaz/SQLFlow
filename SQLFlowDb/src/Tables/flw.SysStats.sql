...
GO
ALTER TABLE [flw].[SysStats] ADD CONSTRAINT [PK_SysStats] PRIMARY KEY CLUSTERED ([StatsID]) ON [PRIMARY]
GO
CREATE NONCLUSTERED INDEX [NCI_FlowId] ON [flw].[SysStats] ([FlowID]) INCLUDE ([Success], [DurationFlow]) ON [PRIMARY]
GO
CREATE NONCLUSTERED INDEX [NCI_FlowType] ON [flw].[SysStats] ([FlowType]) INCLUDE ([FlowID]) ON [PRIMARY]
GO
