...
FROM inserted;
GO
ALTER TABLE [flw].[Invoke] ADD CONSTRAINT [PK_Invoke] PRIMARY KEY CLUSTERED ([FlowID]) ON [PRIMARY]
GO
CREATE NONCLUSTERED INDEX [NCI_FlowID] ON [flw].[Invoke] ([FlowID]) INCLUDE ([Batch], [SysAlias], [DeactivateFromBatch]) ON [PRIMARY]
GO
CREATE UNIQUE NONCLUSTERED INDEX [NCI_UniqueScriptAlias] ON [flw].[Invoke] ([InvokeAlias]) ON [PRIMARY]
GO
