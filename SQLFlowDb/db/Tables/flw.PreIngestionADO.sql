...
INSERT INTO  flw.SysLog (FlowID,[FlowType], [Process])
    SELECT FlowId ,FlowType ,  [srcServer] + '.' + [flw].[GetADOSourceName](FlowId)  + '-->' + [trgServer] + '.' + [trgDBSchTbl] as [Process]
        FROM inserted
GO
ALTER TABLE [flw].[PreIngestionADO] ADD CONSTRAINT [PK_PreIngestionADO] PRIMARY KEY CLUSTERED ([FlowID]) ON [PRIMARY]
GO
CREATE NONCLUSTERED INDEX [NCI_FlowID] ON [flw].[PreIngestionADO] ([FlowID]) INCLUDE ([Batch], [SysAlias], [DeactivateFromBatch]) ON [PRIMARY]
GO
