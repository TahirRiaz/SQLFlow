...
--[flw].[ExecPreFlowXLS] 1
/*
  ##################################################################################################################################################
  -- Name				:   [flw].[GetRVPreFlowXLS]
  -- Date				:   2022-10-28
  -- Author				:   Tahir Riaz
  -- Company			:   Business IQ
  ...
  -- Summary			:	Input parameters:
							...
							@FlowID INT
							@FlowType VARCHAR(50) with default value 'xls'
							...
							and other configuration parameters. It also constructs a dynamic SQL statement to execute the [flw].[ExecPreFlowXLS] function with the appropriate parameters.
  ...
  ##################################################################################################################################################
*/
CREATE PROCEDURE [flw].[GetRVPreFlowXLS]
    @FlowID INT,                      -- FlowID from [flw].[Ingestion]
    @FlowType VARCHAR(50) = 'xls',
...
BEGIN
    ...
    DECLARE @srcFile [NVARCHAR](250),
            ...
            @SyncSchema BIT = 1,
            @SheetName [NVARCHAR](250) = N'',
            @SheetRange [NVARCHAR](250) = N'',
    ...
    SELECT @srcFile = csv.srcFile,
           ...
           @SheetName = ISNULL(SheetName, ''),
           @SheetRange = ISNULL(SheetRange, ''),
           ...
           @FlowType = ISNULL(csv.FlowType, ''),
           @copyToPath = ISNULL(copyToPath, ''),
           @UseSheetIndex = ISNULL(UseSheetIndex, ''),
           ...
           @TrgIndexes = CASE
                             ...
                             ELSE
                                 ''
                         END,
           @IncludeFileLineNumber = ISNULL(IncludeFileLineNumber, 0),
    ...
    BEGIN
        ...
        SELECT @srcPath AS srcPath,
               ...
               @UseSheetIndex AS UseSheetIndex,
               ...
               @IncludeFileLineNumber AS IncludeFileLineNumber,
...
END;
GO
EXEC sp_addextendedproperty N'MS_Description', '-- Purpose			:   This stored procedure builds a metadata dataset for ingesting an Excel file.
  -- Summary			:	Input parameters:
							...
							For example, it fetches values for BulkLoadTimeoutInSek, GeneralTimeoutInSek, BulkLoadBatchSize, NoOfThreads, MaxRetry, RetryDelayMS, 
							and other configuration parameters. It also constructs a dynamic SQL statement to execute the [flw].[ExecPreFlowXLS] function with the appropriate parameters.', 'SCHEMA', N'flw', 'PROCEDURE', N'GetRVPreFlowXLS', NULL, NULL
GO
