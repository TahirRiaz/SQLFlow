...
GO
ALTER TABLE [flw].[SysLogBatch] ADD CONSTRAINT [PK_SysLogBatch] PRIMARY KEY CLUSTERED ([RecID]) WITH (DATA_COMPRESSION = PAGE)
GO
CREATE NONCLUSTERED INDEX [NCI_Batch] ON [flw].[SysLogBatch] ([Batch]) INCLUDE ([BatchID]) WHERE ([Status]<>'Done')
GO
CREATE NONCLUSTERED INDEX [NCI_BatchID] ON [flw].[SysLogBatch] ([BatchID]) INCLUDE ([FlowID], [EndTime])
GO
CREATE NONCLUSTERED INDEX [NCI_EndTime] ON [flw].[SysLogBatch] ([EndTime]) INCLUDE ([BatchID], [Batch], [FlowType], [SysAlias]) WHERE ([EndTime] IS NULL)
GO
CREATE NONCLUSTERED INDEX [NCI_FlowID_EndTime] ON [flw].[SysLogBatch] ([FlowID], [EndTime]) INCLUDE ([Batch], [BatchID])
GO
