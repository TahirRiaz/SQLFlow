...
INSERT INTO  flw.SysLog (FlowID,[FlowType], [Process])
    SELECT FlowId ,FlowType ,  [srcServer] + '.sqlCMD-->' + [trgPath] + '.' + [trgFileName] AS [Process]
        FROM inserted
GO
ALTER TABLE [flw].[Export] ADD CONSTRAINT [PK_Export] PRIMARY KEY CLUSTERED ([FlowID])
GO
CREATE NONCLUSTERED INDEX [NCI_FlowID] ON [flw].[Export] ([FlowID]) INCLUDE ([Batch], [SysAlias], [DeactivateFromBatch])
GO
